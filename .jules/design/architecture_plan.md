...
- **`boksId`**: Derived from the BLE Device Name (e.g., "Boks_A1B2").
  - _Note:_ Web Bluetooth doesn't expose MAC addresses for privacy. We must rely on the advertised name or a characteristic value (like Serial Number if available in Device Info service). For now, we will use the **Device Name** as the primary ID.
