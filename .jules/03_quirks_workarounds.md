...
**Recommendation:** While a strict 5-second lockout is not mandatory, be aware that command responsiveness will be degraded during this time. Avoid auto-triggering heavy tasks (like full Log Sync) _immediately_ (< 1s) after a Door Close event.
...
- **Standard Behavior (Payload Length):** Most opcodes (like `0x79` NOTIFY_LOGS_COUNT) put the number of payload bytes in the 2nd byte.
  - _Example (0x79):_ `79 02 00 17 92` -> Length `02` for 2 bytes of data.
- **The Exception (Total Length):** The opcode `0xC3` (NOTIFY_CODES_COUNT) puts the **Total Packet Length** in the 2nd byte.
  - _Example (0xC3):_ `c3 07 00 02 0c fd d5` -> Length `07` for a 7-byte packet.
