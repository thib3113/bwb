...
*   **Startup:**
    ...
    2.  **Spontaneous Packets:** The Boks often sends `CODES_COUNT` (`0xC3`) or `LOGS_COUNT` (`0x79`) spontaneously upon connection. Update your UI state globally when these arrive.

## 2. Door Control Workflow
*   **Action:** Send `OPEN_DOOR (0x01)` + `PIN_CODE`.
*   **Success:** `VALID_CODE (0x81)` -> Wait for `0x91` (Door Opened).
...
4.  **Streaming:** Process until `LOG_END (0x92)` is received. Calculate event time: `Date.now() - (PacketAge * 1000)`.
...
2.  **Step 2 (Detection):** Wait for `NFC_TAG_FOUND (0xC5)`. Extract the `UID`.
3.  **Step 3 (Register):** Send `NFC_REGISTER (0x18)` + `config_key` + `UID`.
4.  **Confirmation:** Wait for `OP_SUCCESS (0x77)`.
